...
async def on_startup():
    ...
    try:
        # Инициализируем банк Firecrawl ключей
        from app.tools import firecrawl_key_bank
        firecrawl_key_bank.init("/opt/data/firecrawl_keys.json")
        logger.info(f"Firecrawl key bank initialized: {firecrawl_key_bank.active_count()} active keys")

        # Инициализируем банк Apify ключей
        from app.tools import apify_key_bank
        apify_key_bank.init("/opt/data/apify_keys.json")
        logger.info(f"Apify key bank initialized: {apify_key_bank.active_count()} active keys")
