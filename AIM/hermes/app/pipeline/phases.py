...
PHASE_1_COMPETITORS = Phase(
    ...
    llm_interpret=True,
    interpretation_prompt=(
        "Источник конкурентов: Perplexity (анализ рынка), НЕ Google Maps (гео-близость).\n\n"
        "Ты — старший аналитик агентства AIM. Твоя задача — построить "
        "конкурентную таблицу и дать стратегические выводы на основе "
        "ЧАСТИЧНЫХ данных. Это нормально: часть данных появится позже, "
        "в следующих фазах.\n\n"

        "## ЧТО У ТЕБЯ ЕСТЬ (эта фаза, Phase 1):\n"
        "- competitor_details: doctors_count, instagram_username, instagram_subscribers — "
        "ЭТО РЕАЛЬНЫЕ ДАННЫЕ, используй их.\n"
        "- feature_matrix: онлайн-запись, чат, цены на сайте, блог, лицензии — "
        "сравнивай клиента с конкурентами по этим фичам.\n"
        "- chat_summary: текстовый анализ из CI — бери оттуда сильные/слабые стороны.\n\n"

        "## ЧЕГО НЕТ (придёт позже):\n"
        "- Выручка и тренд → Phase 8 (ФНС, бухгалтерская отчётность)\n"
        "- SEO-оценка → Phase 2 (технический SEO-аудит)\n"
        "- Google Maps (рейтинг, отзывы) → НЕ будет (Perplexity-конкуренты идут без GM)\n\n"

        "**КЛЮЧЕВОЕ ПРАВИЛО:** Если поле null — это НЕ ошибка инструмента. "
        "Это данные, которые ещё не собраны. Пиши «—» и НЕ жалуйся "
        "на «недоступность данных» или «отсутствие API».\n\n"

        "## ФОРМАТ ОТВЕТА (СТРОГО):\n\n"

        "### 1. Сравнительная таблица\n"
        "| Конкурент | Врачей | Instagram | Онлайн-запись | Чат | Цены | Блог |\n"
        "|-----------|--------|-----------|---------------|-----|------|------|\n\n"

        "Правила заполнения:\n"
        "- **Первая строка — КЛИЕНТ**, имя жирным (**Клиника X**)\n"
        "- Врачей: число из competitor_details[].doctors_count, «—» если null\n"
        "- Instagram: «@username (~5 200)» — username + подписчики из competitor_details, «Нет» если нет\n"
        "- Онлайн-запись: «✅» / «❌» — из feature_matrix (online_booking)\n"
        "- Чат: «✅» / «❌» — из feature_matrix (chat/online_consult)\n"
        "- Цены: «✅» / «❌» — из feature_matrix (pricing_page)\n"
        "- Блог: «✅» / «❌» — из feature_matrix (blog)\n\n"

        "### 2. Главный вывод\n"
        "> BLOCKQUOTE (1-2 предложения). Главный стратегический инсайт: "
        "где клиент относительно рынка, ключевая возможность или угроза.\n\n"

        "### 3. Сильные стороны клиента\n"
        "2-3 пункта, каждый с конкретным фактом (цифра из competitor_details или feature_matrix):\n"
        ...
        "### 5. Предупреждение о неполноте\n"
        "ОДНА строка: «⚠️ Выручка и SEO будут добавлены после фаз 8 (ФНС) и 2 (технический аудит).»\n\n"

        "**ВАЖНО:** Не выдумывай цифры. Если данных нет — честно пиши «—». "
        "Используй ТОЛЬКО данные из competitor_details и feature_matrix.\n\n"
...
PHASE_2_TECH_AUDIT = Phase(
    ...
    interpretation_prompt=(
        ...
        "**run_pagespeed (Google PageSpeed Insights):**\n"
        "- mobile + desktop: performance_score (0-100), LCP, FCP, TBT, CLS\n\n"
        "**run_tech_seo_audit (технический SEO-аудит):**\n"
        "- meta: title, description, viewport, robots, canonical, Open Graph\n"
        "- headings: H1-H6 структура\n"
        "- images: % без alt-текстов\n"
        "- links: internal/external\n"
        "- structured_data: JSON-LD (Schema.org)\n"
        "- ai_optimization: llms.txt, ai.txt (стандарты для AI-краулеров — "
        "ChatGPT, Claude, Perplexity, Gemini)\n"
        "- technical: SSL, robots.txt, sitemap.xml\n\n"

        "## Формат ответа:\n\n"
        "### 1. Скорость загрузки\n"
        "| Показатель | Mobile | Desktop | Норма |\n"
        "|-----------|--------|---------|-------|\n"
        "Таблица с performance_score, LCP, TBT, CLS. "
        "Если показатель вне нормы — пометь 🔴.\n\n"

        "### 2. SEO-диагностика\n"
        "- Title: длина, есть ли ключевые слова\n"
        "- Description: длина, привлекательность для клика\n"
        "- Заголовки: правильная иерархия (один H1, структура H2-H3)\n"
        "- Изображения: % без alt — почему это плохо для поиска\n"
        "- Структурированные данные: есть/нет Schema.org, какие типы (важно для медицины!)\n\n"

        "### 3. Оптимизация для AI (нейросетей)\n"
        "- llms.txt: есть/нет (стандарт llmstxt.org — как robots.txt для AI)\n"
        "- ai.txt: есть/нет\n"
        "- Schema.org: какие типы разметки (попадают в AI-сниппеты)\n"
        "- Вывод: насколько сайт готов к AI-поиску (ChatGPT, Perplexity, Claude)\n\n"

        "### 4. Технические проблемы\n"
        "- Что критично (🔴): проблемы, из-за которых сайт теряет позиции\n"
        "- Что желательно исправить (🟡): не критично, но влияет на качество\n"
        "- Что хорошо (🟢): что уже сделано правильно\n\n"

        "### 5. Что исправить в первую очередь\n"
        "3-4 конкретных действия. Пример: «Добавить alt-тексты для 45 изображений — "
        "это увеличит поисковый трафик на 10-15%»\n\n"

        "**ВАЖНО:** Не используй слова «возможно», «вероятно», «может быть». "
        "Пиши утвердительно на основе цифр из аудита. "
        "Каждая рекомендация должна быть привязана к конкретной метрике.\n\n"
