...
def _maybe_build_no_instagram_block(
    ...
    try:
        from app.qc_checklist import is_niche_instagram_critical
    except Exception:
...
def _build_qc_coverage_section(metadata: dict) -> str:
    ...
    try:
        from app.qc_checklist import QC_CHECKLIST
...
async def handle_generate_html_report(
    ...
    **kwargs,
    ...
    # Multi-turn assembly: if LLM wrote narrative to a file (e.g.
    # /tmp/{slug}-narrative.md) via several file_write(append=true) calls,
    # read it from disk. This bypasses the ~13K ceiling of single LLM response
    # and allows 50K+ narrative reports.
    narrative_file = kwargs.get("narrative_file")
    if not narrative_file:
        # Also try positional dict (LLM sometimes passes args as dict)
        d = kwargs.get("args_dict") if isinstance(kwargs.get("args_dict"), dict) else {}
        narrative_file = d.get("narrative_file")
    if narrative_file and isinstance(narrative_file, str):
        try:
            from pathlib import Path
            npath = Path(narrative_file)
            if npath.exists():
                file_content = npath.read_text(encoding="utf-8")
                # File wins over inline narrative_md (file is typically longer/more complete)
                if len(file_content) > len(narrative_md or ""):
                    narrative_md = file_content
                    logger.info("generate_html_report: narrative loaded from file %s (%d chars)",
                                narrative_file, len(file_content))
            else:
                logger.warning("generate_html_report: narrative_file not found: %s", narrative_file)
        except Exception as exc:
            logger.warning("generate_html_report: failed to read narrative_file %s: %s",
                           narrative_file, exc)
    ...
    # Framework-level quality gate (v3.3++ Plan A++):
    # If narrative_md is too short (< 15K chars), LLM truncated the report.
    # Instead of saving a half-empty HTML, return coaching feedback to LLM
    # so it can extend via file_write(append=true) + narrative_file path.
    narrative_text = narrative_md or ""
    expected_sections = ["01", "02", "03", "04", "05", "06", "07", "08", "09", "10"]
    found_sections = [s for s in expected_sections if f"## {s} " in narrative_text or f"## {s}-" in narrative_text or f"## {s}—" in narrative_text]
    missing_sections = [s for s in expected_sections if s not in found_sections]
    narrative_too_short = len(narrative_text) < 15000

    if narrative_too_short or missing_sections:
        # Don't save HTML yet — return coaching message so LLM can extend
        coaching = (
            f"⚠️ NARRATIVE INCOMPLETE — HTML not saved yet.\n\n"
            f"Current narrative: {len(narrative_text)} chars (target: 20000+).\n"
            f"Sections found: {found_sections}\n"
            f"Missing sections: {missing_sections}\n\n"
            f"ACTION REQUIRED:\n"
            f"1. Use file_write(file_path='/tmp/{session_hash}-narrative.md', content='<existing narrative>', append=false) to save what you have\n"
            f"2. Then file_write(..., content='<missing sections 04-07 with rich detail>', append=true)\n"
            f"3. Then file_write(..., content='<missing sections 08-10>', append=true)\n"
            f"4. Finally call generate_html_report(narrative_file='/tmp/{session_hash}-narrative.md')\n\n"
            f"This bypasses your 13K response ceiling. Each section should be 1500-3000 chars."
        )
        logger.warning("generate_html_report: narrative too short (%d chars, missing %s) — returning coaching",
                       len(narrative_text), missing_sections)
        return json.dumps({
            "status": "narrative_incomplete",
            "coaching": coaching,
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "narrative_file": {
                        "type": "string",
                        "description": (
                            "Путь к markdown-файлу с полным отчётом. ИСПОЛЬЗУЙ КОГДА narrative_md > 13000 символов "
                            "(GLM-4.6 потолок одного ответа). Процедура: 1) file_write(file_path='/tmp/{slug}-narrative.md', "
                            "content='первые 13000 символов') 2) file_write(..., content='следующие 13000', append=true) "
                            "3) generate_html_report(narrative_file='/tmp/{slug}-narrative.md'). Файл читается с диска — "
                            "нет потолка на размер."
                        ),
                    },
                },
                "required": ["client_url"],
