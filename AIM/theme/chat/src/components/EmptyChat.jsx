...
export function EmptyChat({ onSend }) {
  return (
    <div className="flex flex-col items-center justify-center flex-1 px-4 py-16" style={{animation: 'fadeIn 0.6s ease-out'}}>
      <div className="text-center max-w-lg mb-10">
        ...
        <p className="text-base md:text-lg text-text-muted">
          Проанализирую вашу клинику, конкурентов и рынок. Покажу сколько пациентов вы теряете и как это исправить.
        </p>
      </div>
      <div className="w-full max-w-lg">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-surface-2 border border-border-hairline rounded-lg px-4 py-3 text-sm text-ink placeholder:text-text-subtle focus:outline-none focus:border-accent transition-colors"
            onKeyDown={e => {
          ...
          <button
            onClick={() => {
              const input = document.querySelector('input[placeholder="Введите адрес сайта вашей клиники..."]');
              if (input?.value.trim()) { onSend(input.value.trim()); input.value = ''; }
            }}
            className="shrink-0 w-10 h-10 rounded-md bg-accent text-white flex items-center justify-center hover:brightness-110 active:scale-95 transition-all"
        ...
        <div className="flex flex-wrap gap-2 mt-4 justify-center">
          {QUICK_ACTIONS.map(action => (
            <button key={action} onClick={() => onSend(action)} className="text-xs text-text-muted bg-surface-2 hover:bg-surface-3 rounded-full px-3 py-1.5 transition-colors">
              {action}
            </button>
          ))}
        </div>
