...
function ChatMessageComponent({ role, content, timestamp, isStreaming, contentRef }: ChatMessageProps) {
  ...
  return (
    ...
    >
      <div className={cn('w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-sm font-bold', isAgent ? 'bg-accent text-white' : 'bg-surface-3 text-text-muted')}>
        {isAgent ? 'AI' : 'Вы'}
      </div>
      <div className={cn('rounded-lg px-4 py-3 text-sm leading-relaxed min-w-0 overflow-hidden', isAgent ? 'bg-surface-2 border border-border-hairline text-ink' : 'bg-accent text-white')}>
        {isAgent ? (
