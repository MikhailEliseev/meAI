...
Декоративные геометрические фигуры, которые медленно двигаются при скролле:
...
/* Позиции и скорости разные */
.shape-1 { top: -10%; right: -5%; --float-speed: 0.2; }
.shape-2 { bottom: -15%; left: -8%; --float-speed: 0.35; }
.shape-3 { top: 40%; right: 15%; --float-speed: 0.25; }
```
...
### 1. Hero (100vh, parallax 3-layer)
- Заголовок display + подзаголовок (fade-in entrance)
- Тёмный градиентный фон с геометрическими floating shapes (parallax layers)
- CTA-кнопки: «Записаться» (primary) + «Подобрать специалиста» (outline)
- Статистика: 2009 / 26 / 5,0 (counter animation on scroll)
- Scroll-down indicator (анимированная стрелка)
