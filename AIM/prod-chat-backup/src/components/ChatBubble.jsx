...
export function ChatBubble({ role, content, timestamp, isStreaming, buttons, onSuggestionClick }) {
    ...
    <div className={"flex gap-3 max-w-[85%] " + (isAgent ? "self-start" : "self-end flex-row-reverse")}>
      ...
      <div className={"rounded-lg px-4 py-3 text-sm leading-relaxed min-w-0 overflow-hidden " + (isAgent ? "bg-surface-2 border border-border-hairline text-ink" : "bg-accent text-white")}>
        ...
        {isAgent && buttons && buttons.length > 0 && !isStreaming && (
          <div className="chat-suggestions flex flex-wrap gap-2 mt-3 pt-3" style={{borderTop: "1px solid var(--glass-border, rgba(0,0,0,0.08))"}}>
            {buttons.map((btn, i) => (
              <button
                key={i}
                type="button"
