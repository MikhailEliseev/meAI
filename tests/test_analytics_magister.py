...
@pytest.fixture
def analytics_magister(event_bus, mock_seo_orchestrator, tmp_path):
    """Ads Magister instance with mocked dependencies"""
    return AnalyticsMagister(
...
@pytest.mark.asyncio
async def test_execute_task_routing_campaign_creation(analytics_magister, mock_seo_orchestrator):
    """Test that execute_task routes track_metrics to CI handler"""
    ...
    result = await analytics_magister.execute_task(task)
...
@pytest.mark.asyncio
async def test_handle_metrics_tracking_success(analytics_magister, mock_seo_orchestrator, event_bus):
    ...
    analytics_magister.current_task_id = task.task_id

    result = await analytics_magister._handle_metrics_tracking(task)
...
@pytest.mark.asyncio
async def test_store_analytics_result(analytics_magister, tmp_path):
    ...
    await analytics_magister._store_analytics_result(result)
...
@pytest.mark.asyncio
async def test_timeout_handling(analytics_magister, mock_seo_orchestrator):
    ...
    with patch.object(analytics_magister, '_handle_metrics_tracking') as mock_handler:
        async def timeout_handler(task):
            try:
                await asyncio.wait_for(slow_execution(), timeout=0.1)
            except asyncio.TimeoutError:
                return analytics_magister._create_timeout_result(task, 900)
...
@pytest.mark.asyncio
async def test_validate_analytics_result_success(analytics_magister):
    ...
    validated = analytics_magister._validate_analytics_result(result)

    assert validated == result


@pytest.mark.asyncio
async def test_validate_analytics_result_missing_task_id(analytics_magister):
    ...
    with pytest.raises(ValueError, match="Missing task_id"):
        analytics_magister._validate_analytics_result(result)


@pytest.mark.asyncio
async def test_validate_analytics_result_no_competitors(analytics_magister):
...
@pytest.mark.asyncio
async def test_progress_updates(analytics_magister, event_bus):
    """Test progress update publishing"""
    analytics_magister.current_task_id = "test-task-7"

    await analytics_magister._publish_progress(1, "started", "Phase 1 started")
...
@pytest.mark.asyncio
async def test_orchestrator_not_registered(analytics_magister):
    """Test error when CI orchestrator not registered"""
    # Remove orchestrator
    analytics_magister.orchestrators = {}
...
@pytest.mark.asyncio
async def test_generic_intelligence_fallback(analytics_magister):
    ...
    with patch.object(analytics_magister, 'search_knowledge', new_callable=AsyncMock) as mock_search:
