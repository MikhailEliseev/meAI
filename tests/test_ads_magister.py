...
@pytest.fixture
def ads_magister(event_bus, mock_seo_orchestrator, tmp_path):
...
@pytest.mark.asyncio
async def test_execute_task_routing_campaign_creation(ads_magister, mock_seo_orchestrator):
    """Test that execute_task routes create_campaign to CI handler"""
    ...
    result = await ads_magister.execute_task(task)
...
@pytest.mark.asyncio
async def test_handle_campaign_creation_success(ads_magister, mock_seo_orchestrator, event_bus):
    ...
    ads_magister.current_task_id = task.task_id

    result = await ads_magister._handle_campaign_creation(task)
...
@pytest.mark.asyncio
async def test_store_ads_result(ads_magister, tmp_path):
    ...
    await ads_magister._store_ads_result(result)
...
@pytest.mark.asyncio
async def test_timeout_handling(ads_magister, mock_seo_orchestrator):
    ...
    with patch.object(ads_magister, '_handle_campaign_creation') as mock_handler:
        async def timeout_handler(task):
            try:
                await asyncio.wait_for(slow_execution(), timeout=0.1)
            except asyncio.TimeoutError:
                return ads_magister._create_timeout_result(task, 900)
...
@pytest.mark.asyncio
async def test_validate_ads_result_success(ads_magister):
    ...
    validated = ads_magister._validate_ads_result(result)

    assert validated == result


@pytest.mark.asyncio
async def test_validate_ads_result_missing_task_id(ads_magister):
    ...
    with pytest.raises(ValueError, match="Missing task_id"):
        ads_magister._validate_ads_result(result)


@pytest.mark.asyncio
async def test_validate_ads_result_no_competitors(ads_magister):
...
@pytest.mark.asyncio
async def test_progress_updates(ads_magister, event_bus):
    """Test progress update publishing"""
    ads_magister.current_task_id = "test-task-7"

    await ads_magister._publish_progress(1, "started", "Phase 1 started")
...
@pytest.mark.asyncio
async def test_orchestrator_not_registered(ads_magister):
    """Test error when CI orchestrator not registered"""
    # Remove orchestrator
    ads_magister.orchestrators = {}
...
@pytest.mark.asyncio
async def test_generic_intelligence_fallback(ads_magister):
    ...
    with patch.object(ads_magister, 'search_knowledge', new_callable=AsyncMock) as mock_search:
