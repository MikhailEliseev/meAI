"""Unit tests for Intelligence Magister"""
...
@pytest.fixture
def intelligence_magister(event_bus, mock_ci_orchestrator, tmp_path):
    """Intelligence Magister instance with mocked dependencies"""
    return IntelligenceMagister(
        agent_id="test-intelligence-magister",
        event_bus=event_bus,
        vault_path=tmp_path / "vault",
        database_url="sqlite+aiosqlite:///:memory:",
        orchestrators={"ci": mock_ci_orchestrator}
    )


@pytest.mark.asyncio
async def test_execute_task_routing_competitor_analysis(intelligence_magister, mock_ci_orchestrator):
    """Test that execute_task routes monitor_competitors to CI handler"""
    ...
    result = await intelligence_magister.execute_task(task)

    # Verify CI orchestrator was called
    mock_ci_orchestrator.execute_ci_analysis.assert_called_once()
...
@pytest.mark.asyncio
async def test_handle_competitor_analysis_success(intelligence_magister, mock_ci_orchestrator, event_bus):
    ...
    intelligence_magister.current_task_id = task.task_id

    result = await intelligence_magister._handle_competitor_analysis(task)
...
@pytest.mark.asyncio
async def test_store_ci_result(intelligence_magister, tmp_path):
    ...
    await intelligence_magister._store_ci_result(result)
...
@pytest.mark.asyncio
async def test_timeout_handling(intelligence_magister, mock_ci_orchestrator):
    ...
    mock_ci_orchestrator.execute_ci_analysis = slow_execution
    ...
    with patch.object(intelligence_magister, '_handle_competitor_analysis') as mock_handler:
        async def timeout_handler(task):
            try:
                await asyncio.wait_for(slow_execution(), timeout=0.1)
            except asyncio.TimeoutError:
                return intelligence_magister._create_timeout_result(task, 900)
...
@pytest.mark.asyncio
async def test_validate_ci_result_success(intelligence_magister):
    ...
    validated = intelligence_magister._validate_ci_result(result)

    assert validated == result


@pytest.mark.asyncio
async def test_validate_ci_result_missing_task_id(intelligence_magister):
    ...
    with pytest.raises(ValueError, match="Missing task_id"):
        intelligence_magister._validate_ci_result(result)


@pytest.mark.asyncio
async def test_validate_ci_result_no_competitors(intelligence_magister):
...
@pytest.mark.asyncio
async def test_progress_updates(intelligence_magister, event_bus):
    """Test progress update publishing"""
    intelligence_magister.current_task_id = "test-task-7"

    await intelligence_magister._publish_progress(1, "started", "Phase 1 started")
...
@pytest.mark.asyncio
async def test_orchestrator_not_registered(intelligence_magister):
    """Test error when CI orchestrator not registered"""
    # Remove orchestrator
    intelligence_magister.orchestrators = {}
...
@pytest.mark.asyncio
async def test_generic_intelligence_fallback(intelligence_magister):
    ...
    with patch.object(intelligence_magister, 'search_knowledge', new_callable=AsyncMock) as mock_search:
