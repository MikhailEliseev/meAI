...
@pytest.fixture
def social_magister(event_bus, mock_seo_orchestrator, tmp_path):
    """Ads Magister instance with mocked dependencies"""
    return SocialMagister(
...
@pytest.mark.asyncio
async def test_execute_task_routing_campaign_creation(social_magister, mock_seo_orchestrator):
    """Test that execute_task routes publish_post to CI handler"""
    ...
    result = await social_magister.execute_task(task)
...
@pytest.mark.asyncio
async def test_handle_post_publishing_success(social_magister, mock_seo_orchestrator, event_bus):
    ...
    social_magister.current_task_id = task.task_id

    result = await social_magister._handle_post_publishing(task)
...
@pytest.mark.asyncio
async def test_store_social_result(social_magister, tmp_path):
    ...
    await social_magister._store_social_result(result)
...
@pytest.mark.asyncio
async def test_timeout_handling(social_magister, mock_seo_orchestrator):
    ...
    with patch.object(social_magister, '_handle_post_publishing') as mock_handler:
        async def timeout_handler(task):
            try:
                await asyncio.wait_for(slow_execution(), timeout=0.1)
            except asyncio.TimeoutError:
                return social_magister._create_timeout_result(task, 900)
...
@pytest.mark.asyncio
async def test_validate_social_result_success(social_magister):
    ...
    validated = social_magister._validate_social_result(result)

    assert validated == result


@pytest.mark.asyncio
async def test_validate_social_result_missing_task_id(social_magister):
    ...
    with pytest.raises(ValueError, match="Missing task_id"):
        social_magister._validate_social_result(result)


@pytest.mark.asyncio
async def test_validate_social_result_no_competitors(social_magister):
...
@pytest.mark.asyncio
async def test_progress_updates(social_magister, event_bus):
    """Test progress update publishing"""
    social_magister.current_task_id = "test-task-7"

    await social_magister._publish_progress(1, "started", "Phase 1 started")
...
@pytest.mark.asyncio
async def test_orchestrator_not_registered(social_magister):
    """Test error when CI orchestrator not registered"""
    # Remove orchestrator
    social_magister.orchestrators = {}
...
@pytest.mark.asyncio
async def test_generic_intelligence_fallback(social_magister):
    ...
    with patch.object(social_magister, 'search_knowledge', new_callable=AsyncMock) as mock_search:
