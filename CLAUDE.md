...
**Михаил НЕ доверяет Claude Code.** После 2 месяцев разработки (1 мая → 30 июня 2026) результат = **ноль**. Полная ноль. Ни одного работающего финального продукта за 2 месяца и кучу потраченных денег на токены.

### Жёсткие правила работы с проектом AIM

1. **Никаких планов без 3+ независимых проверок.** Прежде чем предложить — проверить через прямые тесты на сервере.
2. **Любое утверждение = факт из измерения.** Не "я думаю", не "вероятно", не "наверное".
3. **Не доверять своим собственным предыдущим выводам** — перепроверять с нуля.
4. **Каждое изменение = backup + rollback план + 8+ smoke tests.**
...
Михаил уже слышал "всё готово" 10+ раз за 2 месяца. Каждый раз либо не работало, либо ломало другое. **Любой план без исчерпывающей проверки = потеря времени и денег.**

Подробности: `refactor-analysis/00-PROJECT-CONTEXT.md`, `20-MAXIMUM-CHECK-COMPLETE.md`
