...
**Goal**: Transform iamaim.ru chat into a wow-effect sales tool with live progress streaming, real-time commentary, final report page, contact collection, and AIM services assistant
**Mode:** mvp
**Depends on**: Phase 8
**Requirements**: TBD (will be defined during discuss-phase)
**Success Criteria** (what must be TRUE):

  1. Frontend displays real-time `tool-progress` SSE events showing Hermes working through analysis phases (not just text streaming)
  2. Chat shows contextual wow-commentary during analysis ("Ого, главная страница не оптимизирована", "У конкурента доктор с 500K подписчиков") based on discovered data
  3. After analysis completes, chat delivers a link to a dedicated report page (separate HTML page on iamaim.ru) with full presale report
  4. Contact collection flow integrated — captures client name, phone, email at the right moment (after wow-effect, before/after report link)
  5. Hermes acts as AIM services sales assistant — knows pricing page content, recommends relevant services based on analysis findings, includes service suggestions in chat
...
| 9. Chat Pro — Website Chat UX Overhaul | 4/4 | Complete | 2026-06-27 |
