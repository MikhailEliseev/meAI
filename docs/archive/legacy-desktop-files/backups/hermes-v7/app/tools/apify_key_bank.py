"""Apify API key bank with round-robin rotation and exhaustion tracking.

Loads keys from APIFY_KEYS_FILE (default: /opt/data/apify_keys.json).
Falls back to APIFY_API_TOKEN env var if no bank file found.
...
logger = logging.getLogger("apify_key_bank")
...
def init(bank_file: str | None = None):
    ...
    if os.path.exists(bank_file):
        _load(bank_file)
        logger.info(
            "ApifyKeyBank: loaded %d keys (%d active) from %s",
            len(_keys), len(_active_indices), bank_file,
        )
    else:
        logger.info(
            "ApifyKeyBank: no bank file at %s — falling back to APIFY_API_TOKEN env var",
            bank_file,
        )
...
def get_next_key() -> str:
    ...
    # Fallback: env var
    env_key = os.environ.get("APIFY_API_TOKEN", "").strip()
    if env_key:
        return env_key

    raise RuntimeError("No Apify keys available — bank empty and APIFY_API_TOKEN not set")
    ...
    with _lock:
        for k in _keys:
            if k["key"] == token and k["status"] == "active":
                ...
                _save()
                logger.warning(
                    "ApifyKeyBank: key %s exhausted [%s] (%d active remaining)",
                    k.get("label", token[:20]), reason, len(_active_indices),
                )
                return

    logger.debug("ApifyKeyBank: token not found or already exhausted: %s...", token[:20])
...
def get_key_with_fallback() -> str:
    """Return next active key from bank, falling back to APIFY_API_TOKEN env var.
...
def _save():
    ...
    try:
        ...
        logger.warning("ApifyKeyBank: save failed: %s", e)
        ...
        # Credit-exhausted keys: recover if new billing month started
            ...
            if exhausted_year and exhausted_month:
                ...
                if month_changed:
                    ...
                    recovered_credits += 1
                    logger.info(
                        "ApifyKeyBank: recovered key %s (new billing month: %d-%02d)",
                        k.get("label", "?"), now_dt.year, now_dt.month,
                    )

    if recovered_rate or recovered_credits:
        logger.info(
            "ApifyKeyBank: auto-recovered %d rate-limited + %d credit-refreshed keys",
