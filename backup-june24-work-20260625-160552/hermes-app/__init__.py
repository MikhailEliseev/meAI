...
def register_all_tools() -> None:
    ...
    _import_tool("run_lighthouse")  # v8: self-hosted Lighthouse, no Google API
    ...
    _import_tool("find_doctor_handles")
    _import_tool("geo_optimizer_tools")
    _import_tool("finalize_research")

    # === Phase 3.5: Report Generation (3 tools) ===
    _import_tool("publish_scout_report")
    _import_tool("generate_html_report")
    # post_report отключён — LLM путает с generate_html_report (Phase 7 issue)
    # _import_tool("post_report")  # v8: прямой постинг от оркестратора
    _import_tool("read_report_reference")  # v8: референс идеального отчёта

    # === Phase 4: Data Depth (2 new tools, D-10 + D-15) ===
    _import_tool("run_forum_pains")
    _import_tool("run_media_urls")
